...
function handleFormSubmission(event) {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('approxLinkCount').style.display = 'none';
        } else {
...
let timeoutId = null; // Declare timeoutId in a broader scope if not already
...
function checkUrlExistence() {
    ...
    // Send the URL to the backend to check if it already exists
    fetch(`/ccd/check-url/?url=${encodeURIComponent(urlInput)}`)
        ...
        .then(data => {
            if (data.exists) {
                ...
                urlMessage.textContent = ""; // Clear message if URL doesn't exist
                urlMessage.style.color = "";
            }
        })
        .catch(error => {
...
// Add event listener to clear message on input change
document.getElementById('urlInput').addEventListener('input', function() {
    ...
    urlMessage.textContent = ""; // Clear message immediately when typing
    urlMessage.style.color = "";
    // Debounce the check to avoid too many requests while typing
    setTimeout(checkUrlExistence, 300); // Wait 300ms after typing stops
});
